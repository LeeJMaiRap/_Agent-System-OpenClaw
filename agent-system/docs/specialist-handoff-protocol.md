...
Define controlled way for PM Agent to hand work to specialist agents while preserving canonical PM Agent in `agent-system/agents/software/pm-agent`.
...
Use when PM Agent needs proposed specialist support from:
...
Before assigning specialist work, PM Agent must confirm:
...
PM Agent
  ...
  -> PM Agent reviews evidence
  -> QA Agent reviews when needed
  -> PM Agent updates project state or marks blocker
```
...
PM Agent may accept specialist work only when:
