...
def build_framework_report(root: pathlib.Path, report_date: str) -> str:
    ...
    commits = list_git_commits(root, 'agent-system/agents/software/pm-agent')
    git_details = list_git_commits_with_date(root, 'agent-system/agents/software/pm-agent', limit=20)
    ...
    changes = []
    if commits_today:
        changes.append(f'Git activity today: {len(commits_today)} commit(s) in `agent-system/agents/software/pm-agent` scope')
    if reports_today:
